#!/usr/bin/env python3
"""scripts/smart_hierarchical_scene_toc_engine.py

Smart Scene Sub-heading & Hierarchical TOC Engine for All EPUBs:
1. If explicit scene breaks ('***', '* * *', '<hr>') exist -> uses them.
2. If no scene breaks exist in long chapters (or intervals > 25 paragraphs) ->
   smartly divides every 20-30 paragraphs (approx. 5-10 ebook pages) into a new scene.
3. Automatically derives elegant, context-aware Korean sub-headings from opening sentences.
...
6. Runs parallel across 8 CPU processes and syncs 100% to Google Drive #Books.
"""
...
def generate_scene_title(scene_idx: int, opening_text: str) -> str:
    """Generates an elegant Korean scene subheading from opening text."""
    ...
    try:
        ...
        for hf in html_files:
            ...
            if len(explicit_breaks) >= 1:
                ...
                last_sep = None
                for p in paragraphs:
                    if p in explicit_breaks:
                        if curr_p_list:
                            target_scene_start_paras.append((last_sep, curr_p_list[0]))
                            curr_p_list = []
                        last_sep = p
                    elif len(p.get_text(strip=True)) > 5:
                        curr_p_list.append(p)
                if curr_p_list:
                    target_scene_start_paras.append((last_sep, curr_p_list[0]))
            elif len(paragraphs) >= 30:
                # Step 3: No explicit breaks -> Smart 5-10 page (20-25 paragraphs) chunking
                step = 25 # ~5-8 pages
...
def main():
    ...
    # Synchronize to Google Drive #Books
