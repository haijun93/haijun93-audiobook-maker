...
VOICE="${VOICE:-cove}"
STUDY_MAX_SOURCE_CHARS="${STUDY_MAX_SOURCE_CHARS:-3500}"
...
HEARTBEAT_FILE="${HEARTBEAT_FILE:-${WORK_DIR}/chatgpt_web_study_job_heartbeat.json}"
  ...
  cat > "${HEARTBEAT_FILE}" <<EOF
{
  ...
  "pid": $$,
  "stage": "wrapper_start",
  "detail": "study watchdog wrapper initialized"
}
EOF
}
...
while true; do
  reset_wrapper_heartbeat
  log_line "start/resume study audiobook"
  ...
  "${PYTHON_BIN}" -u "${AUDIOBOOK_MAKER_SCRIPT}" \
    ...
    --study-max-source-chars "${STUDY_MAX_SOURCE_CHARS}" \
  ...
  if (( monitor_status == 124 )); then
    log_line "watchdog restart"
    continue
  fi

  if (( exit_code == 0 )); then
    log_line "study audiobook complete"
    break
  fi

  log_line "study audiobook failed with exit=${exit_code}"
  exit "${exit_code}"
done
