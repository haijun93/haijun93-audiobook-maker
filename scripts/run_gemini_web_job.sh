...
GEMINI_WEB_READING_INSTRUCTIONS_FILE="${GEMINI_WEB_READING_INSTRUCTIONS_FILE:-}"
...
HEARTBEAT_FILE="${HEARTBEAT_FILE:-${WORK_DIR}/gemini_web_job_heartbeat.json}"
...
EXTRA_ARGS=()
if [[ -n "${GEMINI_WEB_READING_INSTRUCTIONS_FILE}" ]]; then
  if [[ ! -f "${GEMINI_WEB_READING_INSTRUCTIONS_FILE}" ]]; then
    echo "missing reading instructions file: ${GEMINI_WEB_READING_INSTRUCTIONS_FILE}" >&2
    exit 1
  fi
  GEMINI_WEB_READING_INSTRUCTIONS="$(<"${GEMINI_WEB_READING_INSTRUCTIONS_FILE}")"
  EXTRA_ARGS+=(--gemini-web-reading-instructions "${GEMINI_WEB_READING_INSTRUCTIONS}")
fi
