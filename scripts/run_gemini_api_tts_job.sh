...
GEMINI_API_TTS_MODEL="${GEMINI_API_TTS_MODEL:-gemini-2.5-flash-preview-tts}"
GEMINI_API_TTS_MAX_ATTEMPTS="${GEMINI_API_TTS_MAX_ATTEMPTS:-3}"
GEMINI_API_TTS_READING_INSTRUCTIONS_FILE="${GEMINI_API_TTS_READING_INSTRUCTIONS_FILE:-}"
...
HEARTBEAT_FILE="${HEARTBEAT_FILE:-${WORK_DIR}/gemini_api_tts_job_heartbeat.json}"
...
current_progress_epoch() {
  ...
  candidates=(
    ...
    "${WORK_DIR}"/*.wav(N)
    "${WORK_DIR}"/*.partial.*(N)
    "${WORK_DIR}"/*_prompt.txt(N)
    "${WORK_DIR}"/*_gemini_api_tts.json(N)
...
EXTRA_ARGS=()
if [[ -n "${GEMINI_API_TTS_READING_INSTRUCTIONS_FILE}" ]]; then
  if [[ ! -f "${GEMINI_API_TTS_READING_INSTRUCTIONS_FILE}" ]]; then
    echo "missing reading instructions file: ${GEMINI_API_TTS_READING_INSTRUCTIONS_FILE}" >&2
    exit 1
  fi
  GEMINI_API_TTS_READING_INSTRUCTIONS="$(<"${GEMINI_API_TTS_READING_INSTRUCTIONS_FILE}")"
  EXTRA_ARGS+=(--gemini-api-tts-reading-instructions "${GEMINI_API_TTS_READING_INSTRUCTIONS}")
fi
...
while true; do
  ...
  "${PYTHON_BIN}" -u "${AUDIOBOOK_MAKER_SCRIPT}" \
    ...
    --gemini-api-tts-model "${GEMINI_API_TTS_MODEL}" \
    --gemini-api-tts-max-attempts "${GEMINI_API_TTS_MAX_ATTEMPTS}" \
    "${EXTRA_ARGS[@]}" &
  child_pid=$!
