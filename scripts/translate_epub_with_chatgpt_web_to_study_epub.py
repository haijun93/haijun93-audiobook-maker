...
11. [토익(TOEIC) 700점~만점(990점) 도약 타깃 학습 노트 규칙]
   - 이 교재는 이미 기초 문법과 중학 기본 단어를 숙지한 **토익 700점대 학습자가 만점을 목표로 공부하는 최고급 학습서**입니다.
   - 🚫 [절대 금지 - 중학 기초 단어 추출 금지]: `happy, afraid, hell, words, love, smile, dinner, travel, ticket, teeth, friend, room, hand, look, walk, see, breath, voice, black, sweet` 등 중학교/초급 수준의 평이한 기본 단어는 절대로 학습 노트로 추출하지 마세요.
   - ✅ [엄선 추출 대상 - TOEIC 700점 이상 고난도/만점용 핵심 표현]:
     1) **고급 어휘**: `plummet, subjugate, relinquish, contingency, musty, threshold, flicker, unfixable, inextricably, succumb, poignant, elusive` 등 700점 이상 고득점 어휘.
     2) **고급 숙어, 구동사(Phrasal Verbs) 및 연어(Collocations)**: `bleed dry, cross the threshold, come to terms with, take for granted, at stake` 등.
     3) **다의어의 고난도 문맥 뜻**: 기본 뜻이 아닌 고급/특수 문맥 의미.
   - 💡 [선별적 추출 원칙]: 해당 문장에 700점 이상급 고난도 어휘나 핵심 표현이 없는 평이한 문장은 **억지로 단어를 쥐어짜내지 말고 학습 노트 줄을 과감하게 생략**하세요. (진짜 가치 있는 어휘에만 집중)
   - 형식: 한국어 번역문 바로 다음 줄에 아래 형식으로 작성 (문장당 최대 1~2개 엄선)
※학습: 고급표현1 - 문맥 뜻; 고급표현2 - 문맥 뜻
{guide_section}
{context_section}
