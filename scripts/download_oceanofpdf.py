        ...
        for idx, (raw_title, href) in enumerate(missing_targets, 1):
            ...
            try:
                ...
                if form.count() > 0:
                    ...
                    print("  Triggering EPUB download (waiting for server countdown)...", flush=True)
                    ...
                    try:
                        ...
                        print(f"  Click/Submit note: {exc}", flush=True)

                    # Poll download directory for 35 seconds
                    temp_file = None
                    for _ in range(35):
                        time.sleep(1.0)
                        if captured_downloads:
                            d = captured_downloads[-1]
                            orig_name = d.suggested_filename
                            temp_file = download_dir / orig_name
                            try:
                                d.save_as(str(temp_file))
                                if temp_file.exists() and temp_file.stat().st_size > 5000:
                                    break
                            except Exception:
                                pass
                        # Also check directory directly
                        epubs_in_dir = list(download_dir.glob("*.epub"))
                        if epubs_in_dir and epubs_in_dir[0].stat().st_size > 5000:
                            temp_file = epubs_in_dir[0]
                            break

                    if temp_file and temp_file.exists() and temp_file.stat().st_size > 5000:
                        orig_name = temp_file.name
                        ...
                        print(f"  ❌ Download timed out or file not received for {raw_title}\n", flush=True)
                else:
