...
Create Korean audiobooks from TXT, EPUB, DOCX, and PDF files. Use the local
web interface for everyday work or the CLI for automation and long-running
jobs.
...
Open [http://127.0.0.1:7860](http://127.0.0.1:7860). The interface accepts a
file or pasted text, shows queued and active jobs, preserves work across server
restarts, streams logs, and provides playback and download when generation is
complete.
...
`AUDIOBOOK_WEB_DATA_DIR`. Job metadata is written atomically, and active jobs
...
이 프로젝트는 한국어 `txt`, `epub`, `docx`, `pdf`를 오디오북으로
변환합니다. 처음 사용하는 경우 위 설치 명령을 실행한 뒤
...
진행률과 로그를 확인하고, 완료된 오디오를 바로 재생하거나 내려받을 수
있습니다.
